...
let onlyData = ""; // Define onlyData globally so it's accessible everywhere

document.getElementById("matchForm").addEventListener("submit", function(event) {
    event.preventDefault(); // Prevent page reload

    let GetScoutName = document.getElementById('scoutName').value;
    let matchLevelElement = document.querySelector('input[name="matchLevel"]:checked');
    let GetMatchLevel = matchLevelElement ? matchLevelElement.value : "Not Selected";
    let GetMatchNumber = document.getElementById('matchNumber').value;
    let GetTeamNumber = document.getElementById('teamNumber').value;
    let GetHasAuto = document.getElementById('hasAuto').checked ? "Yes" : "No";
    
    let GetAl1 = document.getElementById('al1').value / 1;
    let GetAl2 = document.getElementById('al2').value / 1;
    let GetAl3 = document.getElementById('al3').value / 1;
    let GetAl4 = document.getElementById('al4').value / 1;
    let GetTakesAlgae = document.getElementById('takesAlgae').checked ? "Yes" : "No";
    let GetAan = document.getElementById('aan').value / 1;
    let GetAap = document.getElementById('aap').value / 1;
    let GetTl1 = document.getElementById('tl1').value / 1;
    let GetTl2 = document.getElementById('tl2').value / 1;
    let GetTl3 = document.getElementById('tl3').value / 1;
    let GetTl4 = document.getElementById('tl4').value / 1;

    let pickupFromElement = document.querySelector('input[name="pickupFrom"]:checked');
    let GetPickupFrom = pickupFromElement ? pickupFromElement.value : "Not Selected";

    let GetTan = document.getElementById('tan').value / 1;
    let GetTap = document.getElementById('tap').value / 1;

    let algaePickupFromElement = document.querySelector('input[name="algaePickupFrom"]:checked');
    let GetAlgaePickupFrom = algaePickupFromElement ? algaePickupFromElement.value : "Not Selected";

    let robotStatusFromElement = document.querySelector('input[name="finalrobotstatus"]:checked');
    let GetFinalRobotStatus = robotStatusFromElement ? robotStatusFromElement.value : "Not Selected";

    let GetDriverSkill = document.getElementById('driverSkill').value;
    let GetDefenseRating = document.getElementById('defenseRating').value;
    let GetSpeedRating = document.getElementById('speedRating').value;
    let GetDied = document.getElementById('died').checked ? "Yes" : "No";
    let GetStabilization = document.getElementById('stabilization').value;
    let GetDroppedCoral = document.getElementById('droppedCoral').value / 1;
    let GetDroppedAlgae = document.getElementById('droppedAlgae').value / 1;
    let GetMakeGoodAlliance = document.getElementById('makeGoodAlliance').value;
    let GetComment = document.getElementById('comment').value;

    let AutoLeaveScore = GetHasAuto === "Yes" ? 3 : 0;
    let AutonomusScore = GetAl1 * 3 + GetAl2 * 4 + GetAl3 * 6 + GetAl4 * 7 + GetAan * 4 + GetAap * 6 + AutoLeaveScore;

    let parkScore = 0;
    if (GetFinalRobotStatus === "parked") {
        parkScore = 2;
    ...
    let TeleOpScore = GetTl1 * 2 + GetTl2 * 3 + GetTl3 * 4 + GetTl4 * 5 + GetTan * 4 + GetTap * 6 + parkScore;

    // Global QR Data String
    ...
    document.getElementById('result').innerText = onlyData;
    generateQRCode();
});
...
function generateQRCode() {
    ...
    let qrUrl = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=" + encodeURIComponent(onlyData) + "&timestamp=" + Date.now();
    qrImage.src = qrUrl;
}
...
document.getElementById("matchForm").addEventListener("submit", function(event) {
    ...
    if(GetHasAuto=="Yes"){
        var AutoLeaveScore = 3;
    }
    else{
        var AutoLeaveScore = 0;
    }
    var AutonomusScore = GetAl1*3 + GetAl2*4 + GetAl3*6 + GetAl4*7 + GetAan*4 + GetAap*6 + AutoLeaveScore;
    ...
    var onlyData = `|| Scout Name: ${GetScoutName} || Match Level: ${GetMatchLevel} || Match Number: ${GetMatchNumber} || 
...
function generateQRCode() {
    qrImage.src = "https://api.qrserver.com/v1/create-qr-code/?size=150x150&data="+ encodeURIComponent(onlyData);
}
